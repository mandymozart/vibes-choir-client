...
async function playSequence(sequence, sequenceNumber) {
    return new Promise(resolve => {
        ...
        const interval = setInterval(() => {
            ...
            console.log(`Sequence ${sequenceNumber + 1}, Step ${step + 1}:`, currentStep);
...
async function loop() {
    ...
    const sequences = Array.from({ length: GROUPS }, (_, i) => generateSequence());

    // Play all sequences concurrently
    await Promise.all(sequences.map((sequence, index) => playSequence(sequence, index)));
