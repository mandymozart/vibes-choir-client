...
function millisecondsWithSinusLFO(minMilliseconds, maxMilliseconds, frequency) {
    ...
    const milliseconds = minMilliseconds + (maxMilliseconds - minMilliseconds) * (sinValue + 1) / 2;
    return milliseconds;
}
...
function randomNumberOfRangeWithWeight(min, max, similarWeight) {
    ...
    const randomWithinSimilarRange = Math.floor(Math.random() * (similarRange + 1));
    const randomOutsideSimilarRange = Math.floor(Math.random() * (diff - similarRange + 1));
...
function weightedRand(weights) {
    ...
    weights = Object.fromEntries(Object.entries(weights).filter(([k, v]) => v !== 0.0));
...
class WeightedList {
    ...
    weightedRand() {
        ...
        return (nextProbability < this._probabilities[nextInt]) ? this._list[nextInt] : this._list[this._alias[nextInt]];
    }

    addWeightToAll(weight) {
        if (weight + this._minWeight <= 0 && this.BadWeightErrorHandling === 'ThrowExceptionOnAdd') {
            throw new Error(`Subtracting ${-1 * weight} from all items would set weight to non-positive for at least one element.`);
        }
    ...
    *[Symbol.iterator]() {
    ...
    toString() {
        let sb = [];
        sb.push(`WeightedList<${this._list[0].constructor.name}>: TotalWeight:${this.totalWeight}, Min:${this.minWeight}, Max:${this.maxWeight}, Count:${this._list.length}, {`);
        for (let i = 0; i < this._list.length; i++) {
    ...
    fixWeight(weight) {
        return (this.BadWeightErrorHandling === 'ThrowExceptionOnAdd') ? this.fixWeightExceptionOnAdd(weight) : this.fixWeightSetToOne(weight);
    }
    ...
    recalculate() {
        ...
        while (small.length > 0 && large.length > 0) {
            ...
            this._alias[l] = g;
            let tmp = scaledProbabilityNumerator[g] + scaledProbabilityNumerator[l] - this._totalWeight;
